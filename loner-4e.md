...
For full procedures covering shared scenes, group conflicts, the Twist Counter across multiple players, and everything else that comes up at the table, see ***Ensemble***, the dedicated group-play expansion for the Loner system.
...
- **Scene structure**
  - **Framing a Scene**: Establish where you are, who is present, and what you intend before the first oracle question.
...
- **Narrative tools**
  ...
  - **Twist Intensity**: Guidance for calibrating how hard a Twist lands based on the fiction's accumulated pressure.
...
- **Long-term mechanics** *(optional modules)*
  - **Challenge Tracks**: A four-box progress tracker for goals that span multiple scenes or sessions.
...
Tags describe what is true in the fiction. That fictional truth is what licenses a roll at all. Before asking whether a tag grants Advantage, ask whether the tag is relevant to this specific fictional moment. A tag can be real and present without bearing on the current situation. Tags provide **context**, not math. They don't work like stats or bonuses; instead, they determine whether a character has an **edge or a setback** in a given situation. If a Tag is relevant, it might grant **Advantage or Disadvantage** on a roll.  
...
- A Protagonist with the Frailty *Cripple* attempts to climb a wall: **Disadvantage**. The same Protagonist asks a guard to let them pass: the visible disability may earn pity, granting **Advantage**. Same tag, different fictional position.
...
Think of Tags as **narrative signposts**: they describe things in a way that directly affects the story.

#### Relationships

Tags describe what a character is. They also describe what a relationship has become.

When a connection with an NPC has been tested, strained, or deepened through play, that relationship earns a tag, *Trusted Informant*, *Sworn Enemy*, *Uneasy Ally*. These tags function like any other: when they bear directly on the situation, they grant Advantage or Disadvantage on the oracle roll.

A relationship tag is not assigned at character creation. It is earned in the fiction. Any scene that shifts how the Protagonist and NPC stand toward each other, a shared risk, a moment of trust, a betrayal, a quiet exchange that changes nothing and everything, can earn a tag. Oracle complications are one source; they are not the only one.

#### 📌 Example: Earning a Relationship Tag

> Zahra asks the corporation's fixer, Rael, to get her past a checkpoint. The oracle answers **No, but...**: he won't help directly, but tips her to a maintenance route the patrols miss.
>
> It's the second time he's declined a direct request while quietly leaving a door open. Zahra assigns the tag *Useful Ambiguity* to the relationship. From now on, when a scene involves Rael and she needs information without a direct ask, that tag is in play as Advantage.
...
4. **Choose a Frailty** – A weakness, flaw, or personal struggle. Its effect depends on the fiction: the same Frailty that hinders in one scene may help in another.  
...
A safehouse can be three tags: *Hidden Entrance*, *Reinforced Door*, *Nosy Neighbors*. If that safehouse becomes a recurring campaign HQ that keeps getting threatened, give it a sheet. The same logic applies to factions, vehicles, and cursed locations, they start as tags and graduate to characters when the fiction demands it.
...
- **Open Threads** – Loose ends that haven't resolved and aren't yet large enough to warrant a Challenge Track: a rumor that went nowhere, a contact who didn't show, a question the oracle left hanging. Just a name and a word or two is enough.
...
Start in **the middle of a tense moment**: your Protagonist is already escaping, fighting, negotiating, or uncovering a secret. The scene itself suggests the next logical action.
...
Use a Dramatic Scene if you want to discover the story as you play. Use the Framed Setup if you want a concrete premise before the first scene. You can combine both: frame the mission, then drop into the middle of it going sideways.
...
- **What** does the Protagonist intend to do here?

You don't need to roll for any of this. Use what the fiction has established: the previous scene's outcome, updated NPC tags, the mood roll that brought you here. If something is uncertain, ask the oracle one framing question before the scene begins and let the answer set the stage. A framing question must be about a fictional condition (what is already true about the situation), not about what will happen. "Is the building still guarded?" is a framing question. "Does Zahra get inside?" is not; that belongs in the scene itself.
...
> - **What:** She intends to lose the tail and find somewhere quiet to examine the stolen datapad.
>
...
4. **Close and Transition** – When the scene ends, note the outcome and roll the **Scene Transition** to open the next one.  
...
- Risk Die higher → **No**
- Both equal → **Yes, but...** (+1 to the Twist Counter)
...
Tags are not numbers. Use them intuitively, not as rigid bonuses or penalties. Ask whether the situation gives an edge or imposes a challenge: don't overanalyze every roll.
...
Zahra is attempting to hack a secure corporate databank. She has no relevant hacking skills, and the system is highly protected: two negative tags with no positives to cancel them. She rolls with Disadvantage (two Risk Dice, one Chance Die).

**Roll:** Chance Die (5), Risk Dice (3) and (4): keep the highest Risk Die (4).

- Chance (5) > Risk (4) → **Yes**
- Both dice ≥ 4 → add **and...**

> Zahra successfully hacks the databank **and** stumbles upon an encrypted transmission revealing an upcoming covert corporate operation.  

![](diagrams/consulting_oracle.png)

### Interpreting the Oracle  

The Oracle provides **a simple answer**. Your interpretation is what makes it specific. Instead of treating the Oracle as just a mechanic, use it as a **narrative driver** that pushes the story forward.  
...
Straightforward answers (*Yes/No*) keep the story moving. Modifiers (*but.../and...*) add complexity and should push the scene in new directions. Tie the answer back to existing story elements, NPCs, locations, unresolved threads, rather than reading it in isolation.  
...
The same result reads differently depending on which method you apply:
...
Don’t treat Oracle results as binary pass/fail. Use context to shape interpretation, a *No, and...* in combat lands differently than in a conversation. Try different methods to keep your story fresh.  
...
Roll 1d6 on each table below (Verb, Noun, and optionally Adjective) to create a prompt. Roll at least a Verb and a Noun. Add an Adjective for more nuance.
...
- On a **No** or **No, and...**, the dead end holds. Shift scale: pull back to the Protagonist's goal and ask what other approach, person, or location might serve it. Start a new scene from there.

If a second dead end follows on the same investigation thread, do not ask the same reframe question again. Instead, shift scale immediately: something about the investigation itself is blocked, not just the current angle. Pull back further, to the goal, to the world, to what the Protagonist knows for certain, and build the next scene from there. Consecutive dead ends are the story's way of telling you the approach needs to change, not just the path.
...
The **Twist Counter** tracks rising tension. Every time you roll doubles, add 1. When the counter reaches 3, a Twist occurs: roll 2d6 on the table below and reset the counter to 0.
...
Interpret the two-word result within the context of the current scene.
...
3. Let the outcome shape the next step in the scene, without tracking Luck loss.  
...
**First Action:** *"Can I take cover behind the wall?"*
**Yes, but...**: She reaches cover, but her line of fire is obstructed (Disadvantage on the next roll).

**Second Action:** *"I aim at the opponent, can I get the first shot in?"*
**No, but...**: She misses, but the bounty hunter is forced to move, breaking his own line of fire (Zahra gains Advantage).

**Final Action:** *"I shoot a slag container above him. Do I hit it?"*
**Yes, and...**: The shot lands perfectly. The container crashes down, knocking the bounty hunter out cold.  
...
Best for named villains, small groups with distinct roles, or rival teams. Resolve their actions individually using separate Luck pools for key adversaries and Oracle rolls to dictate group behavior (*Do they fight as a unit? Do they break formation?*). More tactical, but can slow play if not kept streamlined.  
...
Conflicts should serve the story, not bog it down.

### Harm & Luck

When a conflict involves risk and attrition, track Luck loss to determine the outcome.

Roll as normal to determine if the Protagonist gets what they want. Success deals damage to the opponent; failure deals damage to the Protagonist.

Luck represents resilience, endurance, and fortune. It is scoped to a single conflict: it resets fully when a conflict begins and again when it ends. A returning antagonist starts at 6 regardless of how a previous confrontation ended. Their prior defeat is a story fact, not a depleted resource. A character who runs out of Luck is defeated: but what that means depends on context:
...
>
> Zahra **attacks with a knife** and rolls **(5) (6) [4]** → **Yes, and...**: she deals 3 damage (Thug’s Luck drops from 6 to 3).
>
> The thug **counterattacks with a kick** and rolls **(3) (2) [2]** → **Yes, but...**: Zahra takes 1 damage (her Luck drops from 6 to 5).  
>
...
When a character reaches 0 Luck, stop rolling. The conflict has reached its conclusion. Ask what makes sense for the story:
...
Each filled box is an active tag. It can impose Disadvantage when relevant to the action. When a box is filled, the previous tag is replaced by the new one: *Hurt* becomes *Injured*, not both at once.

When the third box fills, the character is overcome for this conflict. The story determines what that means.

**Recovery**: A consequence tag clears when the fiction supports it: rest, treatment, a quiet scene, or a scene explicitly framed around recovery. Clearing a tag erases one box. Recovery requires narrative justification, not just time passing.

#### 📌 Example: Status Track in Play

> Zahra takes repeated harm in a fight. After losing the conflict her Status Track reads: ☑ ☑ ☐
>
> She escapes with *Injured* active. In the next scene, framed as finding shelter and treating the wound, she clears the tag: ☑ ☐ ☐, now only *Hurt*.
>
...
> Zahra rests at Melina’s apartment after a difficult extraction. She rolls for the next scene’s mood: **6: Meanwhile**.
>
...
Use a Challenge Track when a pursuit spans multiple sessions and a single oracle throw won't resolve it: a vow, a long investigation, a war of attrition.
...
If the scene was framed around something else entirely, leave the track alone: the pursuit wasn't in focus, so nothing changed for it. If the scene contained multiple oracle rolls connected to the challenge, use the result that closed the scene.
...
>
> **Scene 3.** She frames a scene around extracting the decrypted files. She checks the track: 2/4 -- a strong run could close this in two more scenes. She asks: *"Can I pull the files before anyone notices?"* The oracle answers **No, and...**. She narrates what happened: the files are gone and she's burned with Melina. Then she erases a box to mark the cost. ☑ ☐ ☐ ☐
>
...
You can hold only one Leverage at a time. When you bank it, write a short phrase describing what it is. Leverage expires when that description no longer applies in the fiction, or when a Twist occurs. The test is whether the current moment still connects to the moment that generated it. If the story has moved elsewhere, it has already expired. In practice this rarely survives more than a scene or two, but the fiction is the judge, not the scene boundary.
...
> The story takes place in a **mystical land of vast, crumbling empires**, where hidden factions maneuver for power in the shadows. The Protagonist, a wandering mage, is on a quest to seek out ancient knowledge, but in doing so will uncover the truth about a secret cult pulling the strings of history.
...
- **What**: She intends to get through to the quarantine ring and find her contact before the lockdown seals the passage.
...
These procedures formalize practices that were implied in the third edition. They are compatible with any third edition game or *Geared Towards Loner* supplement without modification.

**Framing a Scene**: Before asking the oracle anything, answer three questions in plain terms: Where is the Protagonist? Who is present? What do they intend to do? This replaces the vaguer "Set Your Expectation" step and makes the fiction explicit before dice appear.

**Closing a Scene**: A scene ends when the Protagonist's immediate goal is resolved, blocked, or abandoned. Stop asking oracle questions when any of these conditions is met and roll the Scene Transition. This replaces the less defined "Repeat Until Conflict or Resolution" loop.

**Dead Ends**: When a narrative path collapses entirely, ask the oracle: *"Does something or someone point toward a new way forward?"* Interpret the result and shift scale rather than continuing to question the closed path. The third edition had no equivalent procedure; use this wherever a lead goes cold.
...
**Step 1: Ask the Question**: Pose a closed question requiring resolution. *Example: "Is the door unlocked?"*
...
The more factors working in favor of the character, the stronger the success. More obstacles mean greater chance of complications or failure.

### 4. Guidelines for Interpretation

**Check for a relevant skill or ability**: lean toward Yes if present, No if absent.

**Assess circumstances**: lean toward Yes, but... if favorable; No, but... if unfavorable.

**Consider resources and preparation**: lean toward Yes, and... if well-equipped or prepared; No, and... if ill-equipped or rushed.  
...
### 6. When to Use the Diceless Oracle
...
If you already own polyhedral dice and prefer a single-die roll over a pool, this variant produces equivalent results with a different feel. It is not a mechanical improvement, it is a different aesthetic.
...
2. **💡 2 Skills**: Unique talents or expertise.
3. **⚡ 1 Frailty**: A weakness or flaw.
4. **🎒 2 Gear**: Specialized equipment.
5. **🎯 Goal & Motive**: What they seek and why.
6. **👿 Nemesis (Optional)**: Their main opposition.
...
- **Who** is present or likely to become present?
- **What** does the Protagonist intend to do?
...
- Hold only **one Leverage** at a time. Lost at scene’s end or when a Twist occurs.
- Spend it to:
  - Add **Advantage** to a future roll on the same pursuit: only if you don’t already have Advantage
  - Convert any **No** into **No, but...**
...
- 🗂️ **The world doesn’t reset. Neither should your records of it.**
